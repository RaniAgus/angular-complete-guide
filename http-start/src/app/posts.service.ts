...
export class PostsService {
  postUrl =
    'https://ng-complete-guide-1baa5-default-rtdb.firebaseio.com/posts.json';
  ...
  createAndStorePost(postData: Post): void {
    this.http
      .post<{ name: string }>(this.postUrl, postData, {
      ...
      ); // Si no me suscribo a la respuesta, Angular no va a enviar la consulta
  }

  fetchPosts(): Observable<Post[]> {
    return this.http.get<{ [key: string]: Post }>(this.postUrl).pipe(
      // Es buena practica usar los operadores de rxjs
      map((responseData) => {
  ...
  clearPosts(): Observable<HttpEvent<any>> {
    return this.http
